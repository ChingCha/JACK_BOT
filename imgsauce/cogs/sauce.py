...
import imgsauce.assets
from imgsauce.bot import bot
from imgsauce.config import config, server_api_limit
from imgsauce.helpers import basic_embed, keycap_emoji, keycap_to_int, reaction_check, validate_url
from imgsauce.lang import lang
from imgsauce.models.database import SauceCache, SauceQueries, Servers
from imgsauce.tracemoe import ATraceMoe
...
class Sauce(commands.Cog):
            ...
            if not url:
                await ctx.reply(
                    embed=basic_embed(
                        ...
                        avatar=imgsauce.assets.AVATAR_SILLY
                    )
                )
                return
        ...
        try:
            ...
            await ctx.reply(
                embed=basic_embed(
                    ...
                    avatar=imgsauce.assets.AVATAR_THINKING
                )
            )
            return
    ...
    async def _build_sauce_embed(self, ctx: commands.Context, sauce: GenericSource) -> discord.Embed:
        ...
        """
        embed = basic_embed()
        embed.set_footer(text=lang('Sauce', 'found', member=ctx.author), icon_url=imgsauce.assets.ICON_FOOTER)
        ...
        if sauce.similarity <= 60:
            embed.set_thumbnail(url=imgsauce.assets.AVATAR_THINKING)
            embed.description = lang('Sauce', 'match_title', {'index': sauce.index, 'similarity': sauce.similarity})
            embed.set_footer(text=lang('Sauce', 'found_low_confidence', member=ctx.author), icon_url=imgsauce.assets.ICON_FOOTER)
